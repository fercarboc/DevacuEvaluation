...
export type AuthedView = "dashboard" | "search" | "add" | "subscription";
...
export default function AppShell({
  ...
  const nav = useMemo(
    () => [
      ...
      { view: "search" as const, label: "Consultar", icon: Search },
      { view: "add" as const, label: "Registrar incidencia", icon: PlusCircle },
    ],
    []
  );
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white border-r border-slate-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {nav.map((i) => (
            <NavItem key={i.view} view={i.view} label={i.label} icon={i.icon} />
          ))}
          ...
          >
            <CreditCard className={`w-5 h-5 ${activeView === "subscription" ? "text-white" : "text-slate-400"}`} />
