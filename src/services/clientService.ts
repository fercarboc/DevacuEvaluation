...
function buildGlobalQueryPayload(raw: string) {
  ...
  return { q_raw: q, q_input, strength, allow_weak: strength === "WEAK", email_norm, doc_norm, phone_digits, phone_last9, phone_e164 };
}
...
export async function checkSignalsGlobal(query: string, months = 24): Promise<GlobalSignals> {
  ...
  if (!raw) {
    return {
      match_strength: "MEDIUM", has_matches: false, count_exact: 0, count_bucket: "0",
      risk_level: "NO_CONCLUYENTE", time_window: `${months}M`, message: "Introduce un criterio válido.",
      structured_summary: null, economic_time_window: null, economic_gross_bucket: null,
      economic_gross_label: null, economic_net_bucket: null, economic_net_label: null,
      sources_label: null, last_seen_bucket: null, documents_bucket: null, hotel_category: null, stars_multiplier: null,
    };
  }

  try {
    const row = await callEdge<any>("debacu-eval-check-signals", {
      q_input: payload.q_input, q_raw: payload.q_raw, months, k: 3,
      allow_weak: payload.allow_weak, match_strength_hint: payload.strength,
      email_norm: payload.email_norm, doc_norm: payload.doc_norm,
      phone_digits: payload.phone_digits, phone_last9: payload.phone_last9, phone_e164: payload.phone_e164,
    });

    const count_exact = typeof row?.count_exact === "number" ? row.count_exact
      : typeof row?.countExact === "number" ? row.countExact
    ...
    const avg_stars = typeof row?.avg_stars === "number" ? row.avg_stars
      : typeof row?.avgStars === "number" ? row.avgStars
      : row?.avg_stars ?? row?.avgStars ?? null;

    const top_typologies = Array.isArray(row?.top_typologies) ? row.top_typologies
      : Array.isArray(row?.topTypologies) ? row.topTypologies : [];

    return {
      match_strength, has_matches, count_exact,
      count_bucket: (row?.count_bucket ?? row?.countBucket ?? "0") as CountBucket,
      avg_stars, risk_level, top_typologies,
      ...
      hotel_category: typeof row?.hotel_category === "number" ? row.hotel_category
        : typeof row?.hotelCategory === "number" ? row.hotelCategory
        : row?.hotel_category ?? row?.hotelCategory ?? null,
      stars_multiplier: typeof row?.stars_multiplier === "number" ? row.stars_multiplier
        : typeof row?.starsMultiplier === "number" ? row.starsMultiplier
        : row?.stars_multiplier ?? row?.starsMultiplier ?? null,
      structured_summary: (row?.structured_summary ?? null) as StructuredSummary | null,
    };
  } catch (e) {
    console.error("checkSignalsGlobal failed:", e);
    return {
      match_strength: payload.strength, has_matches: false, count_exact: 0, count_bucket: "0",
      risk_level: "NO_CONCLUYENTE", time_window: `${months}M`,
      message: "No se ha podido completar la comprobación.", structured_summary: null,
      economic_time_window: null, economic_gross_bucket: null, economic_gross_label: null,
      economic_net_bucket: null, economic_net_label: null, sources_label: null,
      last_seen_bucket: null, documents_bucket: null, hotel_category: null, stars_multiplier: null,
    };
  }
}
...
export async function searchMyRatingsInSupabase(q: string, limit = 50): Promise<MyRatingsSearchResponse> {
  ...
  const signals: MyRatingsSignals | null = s
    ? {
        ...
        time_window: "MINE",
        top_typologies: Array.isArray(s?.top_typologies) ? s.top_typologies
          : Array.isArray(s?.topTypologies) ? s.topTypologies : [],
...
/** =========================================================
 *  INSERT evaluation
 *  ✅ FIX: customer_id y property_id ahora incluidos en payload
 * ========================================================= */
...
export async function addEvaluation(
  ...
  const payload: EvaluationInsert = {
    ...
    // ✅ Campos obligatorios añadidos para satisfacer EvaluationInsert
    customer_id: currentCustomerId ?? "",
    property_id: propertyId ?? "00000000-0000-0000-0000-000000000000",
...
export type GlobalRiskSnapshot = {
  pct_5: number; pct_4: number; pct_3: number; pct_2: number; pct_1: number;
  pct_bajo: number; pct_medio: number; pct_alto: number;
};
...
/** =========================================================
 *  client_dashboard v1 (legacy fallback)
 * ========================================================= */
export type ClientDashboardData = {
  ...
  activity: Array<{
    ...
    label: string; contact: string; rating: number | null;
  }>;
};
...
export type AuditHistoryItem = {
  audit_id: string; created_at: string; event_type: string | null;
  type: string | null; label: string | null; type_label: string | null;
  risk_level: string | null; avg_stars: number | null;
  match_strength: string | null; count_bucket: string | null;
  count_exact: number | null; time_window: string | null;
  input_kind: string | null; search_value_masked: string | null;
  result_count: number | null; actor_user_id: string | null;
  actor_role: string | null; actor_email: string | null; meta: any;
};

export type ListAuditHistoryResp = {
  items: AuditHistoryItem[]; total: number; page: number; page_size: number;
};
...
function normalize_audit_item(row: any): AuditHistoryItem {
  ...
  const type_label = label ?? type ?? (event_type
    ? (String(event_type).toUpperCase() === "CHECK_SIGNALS" ? "Consulta" : event_type)
    : null);
  const risk_level = (row?.risk_level ?? row?.risk ?? meta?.risk_level ?? meta?.risk ?? null) as string | null;
  const avg_stars = typeof row?.avg_stars === "number" ? row.avg_stars
    : typeof row?.rating === "number" ? row.rating
    : typeof meta?.avg_stars === "number" ? meta.avg_stars
    : typeof meta?.rating === "number" ? meta.rating
    : row?.avg_stars ?? row?.rating ?? meta?.avg_stars ?? meta?.rating ?? null;
  const match_strength = (row?.match_strength ?? row?.matchStrength ?? meta?.match_strength ?? meta?.matchStrength ?? null) as string | null;
  const count_bucket = (row?.count_bucket ?? row?.countBucket ?? meta?.count_bucket ?? meta?.countBucket ?? null) as string | null;
  const count_exact = typeof row?.count_exact === "number" ? row.count_exact
    : typeof row?.countExact === "number" ? row.countExact
    : typeof meta?.count_exact === "number" ? meta.count_exact
    : typeof meta?.countExact === "number" ? meta.countExact
    : row?.count_exact ?? row?.countExact ?? meta?.count_exact ?? meta?.countExact ?? null;
  const time_window = (row?.time_window ?? row?.timeWindow ?? meta?.time_window ?? meta?.timeWindow ?? meta?.months_received ?? null) as string | null;
  const input_kind = (row?.input_kind ?? row?.search_kind ?? meta?.input_kind ?? meta?.search_kind ?? null) as string | null;
  const search_value_masked = (row?.search_value_masked ?? row?.contact ?? meta?.search_value_masked ?? meta?.contact ?? null) as string | null;
  const result_count = typeof row?.result_count === "number" ? row.result_count
    : typeof row?.resultCount === "number" ? row.resultCount
    : typeof meta?.result_count === "number" ? meta.result_count
    : typeof meta?.resultCount === "number" ? meta.resultCount
  ...
  const actor_email = (row?.actor_email ?? row?.requested_by_email ?? meta?.actor_email ?? meta?.requested_by_email ?? null) as string | null;
  return { audit_id, created_at, event_type, type, label, type_label, risk_level, avg_stars, match_strength, count_bucket, count_exact, time_window, input_kind, search_value_masked, result_count, actor_user_id, actor_role, actor_email, meta };
}
...
function normalize_list_audit_history(raw: any, fallback: { page: number; page_size: number }): ListAuditHistoryResp {
  ...
  return { items: items_raw.map(normalize_audit_item), total, page, page_size };
}

export async function list_audit_history(params: {
  page: number; page_size: number;
  q?: string | null; event_type?: string | null;
  date_from?: string | null; date_to?: string | null;
}): Promise<ListAuditHistoryResp> {
  const payload = {
    page: params.page, page_size: params.page_size,
    q: params.q ?? null, event_type: params.event_type ?? "CHECK_SIGNALS",
    date_from: params.date_from ?? null, date_to: params.date_to ?? null,
  };
...
export type IssueAuditPdfResp = {
  pdf_event_id: string; pdf_event_created_at: string | null;
  download_url: string | null; storage_path: string | null;
};
...
export async function issue_audit_pdf(params: { source_audit_id: string; template_version?: string }) {
  ...
  return {
    ...
    pdf_event_created_at: raw?.pdf_event_created_at ?? null,
...
export async function audit_export_generate(payload: {
  export_type: "PDF" | "CSV"; export_scope: string;
  period_from: string; period_to: string;
  filters?: Record<string, unknown>; source_audit_id?: string | null;
}) {
...
export type WeeklySeriesRow = {
  day: string; incidents: number; risk_high: number; risk_medium: number;
  risk_low: number; gross: number; recovered: number; net: number;
};

export async function getWeeklySeries7d(args: {
  period_from: string; period_to: string; period_field: PeriodField;
}): Promise<WeeklySeriesRow[]> {
  const resp: any = await callEvalFn("customer_operational_weekly_series_get", {
    period_from: args.period_from, period_to: args.period_to, period_field: args.period_field,
  });
...
export type GlobalGuestLookupResp = {
  exists: boolean; risk_band: "LOW" | "MEDIUM" | "HIGH" | null;
  stays_count: number; reservations_count: number; incidents_count: number;
  total_gross: number; total_recovered: number; total_net_loss: number;
  first_seen_date: string | null; last_seen_date: string | null;
  last_incident_date: string | null; input_kind: "DOC" | "EMAIL" | "PHONE";
};
...
/** =========================================================
 *  DASHBOARD V2 — tipos multi-propiedad
 * ========================================================= */
...
export type UsageSummary = {
  plan_query_total: number;
  manual_query_count: number;
  csv_screening_count: number;
  csv_revenue_count: number;
  created_this_month: number;
};

export type PropertyComparisonRow = {
  property_id: string; property_name: string;
  incidents_count: number; gross_loss: number;
  recovered: number; net_loss: number;
  risk_high_count: number; revenue_impacted: number;
};

export type UpcomingRiskAlert = {
  id: string; checkin_date: string;
  property_id: string; property_name: string;
  risk_band: "HIGH" | "MEDIUM" | "LOW";
  identity_key: string; batch_ref: string | null;
  incidents_count: number | null; total_net_loss: number | null;
};

export type ClientDashboardV2 = {
  customer_id: string; month_start: string;
  org_summary: OrgSummary; usage_summary: UsageSummary;
  properties: PropertyInfo[];
  property_comparison: PropertyComparisonRow[];
  upcoming_risk_alerts: UpcomingRiskAlert[];
};

function normalizeClientDashboardV2(raw: any): ClientDashboardV2 {
  return {
    customer_id: String(raw?.customer_id ?? ""),
    month_start: String(raw?.month_start ?? ""),
    org_summary: raw?.org_summary ?? { plan_card: null },
    usage_summary: {
      plan_query_total: Number(raw?.usage_summary?.plan_query_total ?? 0),
      manual_query_count: Number(raw?.usage_summary?.manual_query_count ?? 0),
      csv_screening_count: Number(raw?.usage_summary?.csv_screening_count ?? 0),
      csv_revenue_count: Number(raw?.usage_summary?.csv_revenue_count ?? 0),
      created_this_month: Number(raw?.usage_summary?.created_this_month ?? 0),
    },
    properties: Array.isArray(raw?.properties) ? raw.properties : [],
    property_comparison: Array.isArray(raw?.property_comparison) ? raw.property_comparison : [],
    upcoming_risk_alerts: Array.isArray(raw?.upcoming_risk_alerts) ? raw.upcoming_risk_alerts : [],
  };
}

function buildV2FromV1(v1: ClientDashboardData): ClientDashboardV2 {
  return {
    customer_id: v1.customer_id,
    month_start: v1.month_start,
    org_summary: { plan_card: v1.plan_card },
    usage_summary: {
      plan_query_total: v1.query_count,
      manual_query_count: v1.query_count,
      csv_screening_count: 0,
      csv_revenue_count: 0,
      created_this_month: v1.created_this_month,
    },
    properties: [],
    property_comparison: [],
    upcoming_risk_alerts: [],
  };
}

export async function getClientDashboardV2(): Promise<ClientDashboardV2> {
  try {
    // ✅ Nombre actualizado a debacu_eval_*
    const raw = await callEdge<any>("debacu_eval_client_dashboard_v2", {});
    return normalizeClientDashboardV2(raw);
  } catch {
    // Fallback al endpoint v1 mientras se migra
    const v1 = await getClientDashboard();
    return buildV2FromV1(v1);
  }
}