...
const channelData = [
  ...
  { name: 'OTA', value: 30, color: '#8B5CF6' },
  { name: 'B2B', value: 15, color: '#10B981' },
      ...
      <div className="flex h-[600px]">
        ...
        <div className="w-16 md:w-56 border-r border-white/[0.08] flex flex-col p-4 gap-6">
          <div className="flex items-center gap-3 px-2 mb-4">
            ...
            <span className="font-bold text-sm hidden md:block">debacu</span>
          </div>
            ...
            {[
              ...
              <div 
                ...
                className={cn(
                  ...
                  item.active ? "bg-blue-600/10 text-blue-400" : "text-slate-400 hover:bg-white/5 hover:text-white"
                )}
              >
                {item.icon}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <header className="h-16 border-b border-white/[0.08] flex items-center justify-between px-6">
            <div className="flex items-center gap-4 flex-1">
              <div className="relative max-w-xs w-full hidden md:block">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-full py-1.5 pl-9 pr-4 text-xs focus:outline-none focus:border-blue-500/50 transition-colors"
                />
              </div>
            </div>
            <div className="flex items-center gap-4">
              ...
              <Bell size={16} className="text-slate-400 cursor-pointer hover:text-white" />
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                { label: 'Monthly Revenue', value: '€125,430', trend: '+14.2%', color: 'text-blue-400' },
                { label: 'Avg. Daily Rate', value: '€142.50', trend: '+5.1%', color: 'text-emerald-400' },
                { label: 'RevPAR', value: '€118.20', trend: '+8.4%', color: 'text-violet-400' },
                { label: 'Occupancy Rate', value: '84.2%', trend: '-2.1%', color: 'text-slate-400' },
              ].map((kpi, i) => (
                <div key={i} className="glass-card p-4 border-white/[0.05] bg-white/[0.01]">
                  <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">{kpi.label}</p>
                  <div className="flex items-baseline justify-between">
                    <h4 className="text-xl font-bold">{kpi.value}</h4>
                    <span className={cn("text-[10px] font-bold", kpi.trend.startsWith('+') ? 'text-emerald-400' : 'text-rose-400')}>
                      {kpi.trend}
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 glass-card p-5 bg-white/[0.01]">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400">Revenue Performance</h3>
              ...
              <div className="glass-card p-5 bg-white/[0.01]">
                <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-6">Risk Indicators</h3>
                <div className="space-y-4">
                  <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20">
                    <div className="flex items-center gap-2 mb-1">
                      <ShieldAlert size={14} className="text-rose-500" />
                      <span className="text-[10px] font-bold text-rose-500 uppercase">Critical Risk</span>
                    </div>
                    <p className="text-[11px] text-slate-300">Unusual cancellation spike in Segment B (OTA)</p>
                  </div>
                  
                  <div className="space-y-3">
                    {[
                      { label: 'Operational Alerts', value: 12, color: 'bg-amber-500' },
                      { label: 'Revenue Leakage', value: 4, color: 'bg-rose-500' },
                      { label: 'Guest Sentiment', value: 94, color: 'bg-emerald-500', isPercent: true },
                    ].map((item, i) => (
                      <div key={i}>
                        <div className="flex justify-between text-[10px] mb-1">
                          <span className="text-slate-400">{item.label}</span>
                          <span className="font-bold">{item.value}{item.isPercent ? '%' : ''}</span>
                        </div>
                        <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                          <div 
                            className={cn("h-full rounded-full", item.color)} 
                            style={{ width: `${item.isPercent ? item.value : (item.value / 20) * 100}%` }} 
                          />
                        </div>
                      </div>
                    ))}
