// supabase/functions/debacu_eval_revenue_seasons_manage/index.ts
...
const SELECT_FIELDS = `
  id, org_id, property_id, name, season_type,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "DELETE") {
      ...
      if (fetchErr || !existing?.id) return json(req, 404, { ok: false, error: "season_not_found" });
    ...
    if (action === "CREATE") {
      ...
      const { data, error } = await sb
        ...
        .insert({
          ...
          name,
          season_type: cleanText(body.season_type),
          start_date: body.start_date!,
          end_date: body.end_date!,
          color: cleanText(body.color) ?? "#3B82F6",
    ...
    if (body.season_type !== undefined) payload.season_type = cleanText(body.season_type);
    ...
    console.error("[debacu_eval_revenue_seasons_manage]", err);
