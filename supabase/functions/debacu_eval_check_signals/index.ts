...
type Scope = "GLOBAL" | "MY"; // GLOBAL = índice global, MY = filtro customer_id (mis registros)
...
// 👇 GLOBAL PEPPER: solo necesario para consultas GLOBAL (índice)
// Si no está, la rama GLOBAL fallará (controlado).
 ...
 * está guardado con o sin prefijo 34 (solo para MY, tabla debacu_evaluations).
 */
...
/**
 * Para GLOBAL (índice): necesitamos un "phone canonical" consistente con el import.
 * Preferimos phone_e164 si llega, si no digits, si no last9.
 */
function normalizePhoneCanonicalFromPayload(body: any): string {
  const e164 = safeStr(body?.phone_e164);
  const digits = safeStr(body?.phone_digits);
  const last9 = safeStr(body?.phone_last9);

  const pick = e164 || digits || last9;
  if (!pick) return "";

  // normalización fuerte: mayúsculas + sin espacios + sin guiones + solo dígitos/+ si aplica
  // para phone, nos quedamos con dígitos y + si existe al inicio
  const t = pick.trim();
...
/* ======================================================
 * Riesgo (MY): simple basado en count + avgStars
 * ====================================================== */
...
/* ======================================================
 * Riesgo (GLOBAL): viene del índice (risk_band)
 * ====================================================== */
function mapGlobalRiskBand(riskBand: any): Risk {
  const v = String(riskBand ?? "").trim().toUpperCase();
  if (!v) return "NO_CONCLUYENTE";
  if (v === "HIGH" || v === "ALTO") return "ALTO";
  if (v === "MEDIUM" || v === "MEDIO") return "MEDIO";
...
/* ======================================================
 * GLOBAL identity_key (HMAC SHA-256)
 * - Debe ser idéntico al import CSV.
 * - Usa doc|email|phone (canon)
 * ====================================================== */
async function hmacIdentityKey(doc: string, email: string, phone: string): Promise<string> {
  if (!GLOBAL_PEPPER) throw new Error("missing_env:DEBACU_GLOBAL_PEPPER");
  const enc = new TextEncoder();
  const data = enc.encode(`${doc}|${email}|${phone}`);
...
/* ======================================================
 * Query builders (MY: debacu_evaluations)
 * ====================================================== */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ======================================================
    // ✅ GLOBAL: índice global (debacu_eval_import_guest_index)
    // ======================================================
    if (scope === "GLOBAL") {
      // Preferir normalizados del payload (tu UI ya los envía)
      const docFromPayload = safeStr(body?.doc_norm);
      const emailFromPayload = safeStr(body?.email_norm);
      const phoneCanon = normalizePhoneCanonicalFromPayload(body);

      // Si no vienen, derivar desde q_raw usando heurísticas ya existentes
      const kind = detectKind(q_raw);
      const doc = normalizeDoc(docFromPayload || (kind === "doc" ? q_raw : ""));
      const email = normalizeEmail(emailFromPayload || (kind === "email" ? q_raw : ""));
      const phone = phoneCanon || (kind === "phone" ? normalizePhoneCanonicalFromPayload({ phone_digits: q_raw }) : "");
      ...
      if (!doc && !email && !phone) {
        return json(req, 200, {
          ...
          data: {
            ...
            avgStars: null,
            message:
              "Resultado no concluyente: para comprobación global añade email/teléfono/documento.",
            k,
          },
        });
      }
      ...
      // identity_key
      const identityKey = await hmacIdentityKey(doc, email, phone);

      // lookup índice
      const { data: idx, error: idxErr } = await sbAdmin
        .from("debacu_eval_import_guest_index")
        .select("risk_band, stays_count, incidents_count, total_net_loss, last_seen_date, last_incident_date")
        .eq("identity_key", identityKey)
        .maybeSingle();

      if (idxErr) return errResp(req, 500, "query_failed");

      const exists = !!idx;
      const stays = Number(idx?.stays_count ?? 0);
      const incidents = Number(idx?.incidents_count ?? 0);
      const countExact = exists ? (stays + incidents) : 0;
      const countBucket = bucketizeCount(countExact);
      const hasMatches = countExact > 0;

      const risk = exists ? mapGlobalRiskBand(idx?.risk_band) : ("NO_CONCLUYENTE" as Risk);

      let message = "";
      if (exists && incidents === 0 && stays > 0) message = "SIN INCIDENCIAS REGISTRADAS ✅";
      if (exists && incidents > 0) message = "Incidencias registradas globalmente.";

      // Audit (RGPD: bucket mínimo)
      ...
      // Response compatible con UI actual
      return json(req, 200, {
        ok: true,
        data: {
          scope,
          ...
          countExact,
          countBucket,
          avgStars: null, // GLOBAL no tiene avgStars (no inventar)
          risk,
          topTypologies: [],
          timeWindow: `${months}M`,
          message,
          k,
          // ✅ opcional: datos agregados adicionales (la UI puede ignorarlo)
          globalIndex: idx
            ? {
              stays_count: stays,
              incidents_count: incidents,
              total_net_loss: Number(idx.total_net_loss ?? 0),
              last_seen_date: idx.last_seen_date ?? null,
              last_incident_date: idx.last_incident_date ?? null,
            }
            : null,
        },
      });
    ...
    // ======================================================
    // ✅ MY: comportamiento actual (debacu_evaluations con customer_id)
    // ======================================================
    const kind = detectKind(q_raw);
