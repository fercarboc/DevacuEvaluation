...
type ReqBody = {
  ...
  from?: string; // yyyy-mm-dd (filter_from >= from)
  to?: string; // yyyy-mm-dd (filter_to <= to)
};
...
type EntitlementsRow = { org_id: string; subscription_status: string | null; plan_code: string | null };
...
export default Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ✅ por defecto: bloquear listado si plan no está activo (como pedías)
    // Si quieres permitir histórico, quita estas 2 líneas.
    const ent = await loadEntitlementsOrThrow(admin, org_id);
    assertOrgActiveOrThrow(ent);

    // tabla “definitiva”
    const TABLE = "debacu_eval_audit_exports";
    // si todavía estás usando customer_audit_exports, cambia a:
    // const TABLE = "customer_audit_exports";
    ...
    let q = admin
      ...
      .eq("org_id", org_id)
      // si tienes app_code en tabla:
      .eq("meta->>app_code", APP_CODE)
      .order("created_at", { ascending: false })
      .range(offset, offset + limit - 1);

    if (status !== "ALL") q = q.eq("status", status);
    // en tabla definitiva el tipo es format (PDF/CSV)
    ...
    // Normaliza salida (shape estable)
